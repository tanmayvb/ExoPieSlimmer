...
def Analyze():
    ...
    DCSVMWP=0.6324
    _pt=[100.0,200.0,400.0,600.0,7000.0]
    NEntries = skimmedTree.GetEntries()

    h_higgs_pT                 =TH1F('h_higgs_',  'h_higgs_Pt',  50,0.0,1500.)
    h_reghiggs_pT              =TH1F('h_reghiggs_',  'h_reghiggs_Pt',  50,0.0,1500.)

    h_Mbb_x                 =TH1F('h_Mbb_x',  'h_Mbb_x',  25,100.,150.)
    h_regMbb_x                 =TH1F('h_regMbb_x',  'h_regMbb_x',  25,100.,150.)
    ...
    for i in range (len(_pt)-1):
        namex= 'Mbb_'+str(i)
        titlex= 'hist_Mbb_with_range_'+str(_pt[i])+"to"+str(_pt[i+1])
        mbbx= TH1F(namex,  titlex,  25,100.,150.)
	h_Mbb.append(mbbx)
        namey= 'regMbb_'+str(i)
        titley= 'hist_regMbb_with_range_'+str(_pt[i])+"to"+str(_pt[i+1])
...
#        thinJetdeepCSV             = skimmedTree.__getattr__('AK4deepCSVjetDeepCSV_b')
   ...
   #--------------------------------------------------
    ...
    #----------------------------------------------------
	    ...
	    #print "pt higgs", addJet.Pt() 
            if(addJet.M() > 100 and addJet.M()<150):
                h_higgs_pT.Fill(higgs_pt)
                h_Mbb_x.Fill(addJet.M())
                for i in range (len(_pt)-1):
                    if higgs_pt>_pt[i] and higgs_pt<_pt[i+1]:
	    ...
	    Corrhiggs_pt=addJetCorr.Pt()
            if(addJetCorr.M() > 100 and addJetCorr.M()<150):
                h_reghiggs_pT.Fill(Corrhiggs_pt)
                h_regMbb_x.Fill(addJetCorr.M())
	        for i in range (len(_pt)-1): 
                    if Corrhiggs_pt>_pt[i] and Corrhiggs_pt<_pt[i+1]:
		        h_regMbb[i].Fill(addJetCorr.M())
      
    f1 = TF1("f1", "gaus",  100, 150);
    f2 = TF1("f2", "gaus",  100, 150);
    c      = SetCanvas()
    legend = CreateLegend(0.4, 0.64, 0.6, 0.92, "")
    legend.SetTextSize(0.03)
    h_higgs_pT.SetMarkerColor(2)
    h_higgs_pT.SetMarkerStyle(20)
    h_reghiggs_pT.SetMarkerColor(4)
    h_reghiggs_pT.SetMarkerStyle(20)
    legend.AddEntry(h_higgs_pT,"Higgs pT")
    legend.AddEntry(h_reghiggs_pT,"Higgs pT withh Mbb Correction")
    h_higgs_pT.SetMarkerSize(3)
    h_reghiggs_pT.SetMarkerSize(3)
    h_higgs_pT.Draw("P")
    h_reghiggs_pT.Draw("P Same")
    legend.Draw("same")
    c.SaveAs("Higgs_PT.pdf")
    
    c0      = SetCanvas()   
    legend1 = CreateLegend(0.13, 0.64, 0.45, 0.92, "")
    legend1.SetTextSize(0.03)
    h_Mbb_x.SetLineColor(2)
    h_regMbb_x.SetLineColor(3)
    h_Mbb_x.SetMarkerSize(3)
    h_regMbb_x.SetMarkerSize(3)

    norm_h_Mbb_x = h_Mbb_x.Integral();
    h_Mbb_x.Scale(1/norm_h_Mbb_x)
    h_Mbb_x.Draw('P')
    h_Mbb_x.Fit("f1","","",100,150)
    f1.SetLineColor(2)
    f1.Draw('same')

    norm_h_regMbb_x = h_regMbb_x.Integral();
    h_regMbb_x.Scale(1/norm_h_regMbb_x)
    h_regMbb_x.Draw('same P')
    h_regMbb_x.Fit("f2","","",100,150)
    f2.SetLineColor(3)
    f2.Draw('same')
    ...
    mbb_entry="Baseline+selection"+"; Entries="+str(int(norm_h_Mbb_x))
    regmbb_entry="Baseline+selection+DNN"+"; Entries="+str(int(norm_h_regMbb_x))

    legend1.AddEntry(h_Mbb_x,mbb_entry)
    legend1.AddEntry(f1,f1_fitvalues)
    legend1.AddEntry(h_regMbb_x,regmbb_entry)
    legend1.AddEntry(f2,f2_fitvalues)
    legend1.Draw()
    c0.SaveAs("Mbb_withot_PtSlice.pdf")
    ...
    for i in range (len(_pt)-1):
        ...
        legend.SetTextSize(0.03)
        h_Mbb[i].SetLineColor(2)
        h_Mbb[i].SetMarkerColor(2)
        h_Mbb[i].SetMarkerSize(1.5)
        h_Mbb[i].SetLineWidth(3)
        h_Mbb[i].SetMarkerStyle(23)
        h_Mbb[i].SetXTitle("M_{bb}")
        h_Mbb[i].SetTitle("")
        norm_h_Mbb =0.
        norm_h_Mbb = h_Mbb[i].Integral();
        h_Mbb[i].Scale(1/norm_h_Mbb)
	c1.cd()
        '''h_Mbb[i].Draw('P')
        h_Mbb[i].Fit("f1","","",100,150)
        f1.SetLineColor(2)
        f1.Draw('same')
        '''
        h_regMbb[i].SetLineColor(3)
        h_regMbb[i].SetMarkerColor(3)
        h_regMbb[i].SetMarkerSize(1.5)
        h_regMbb[i].SetLineWidth(3)
        h_regMbb[i].SetMarkerStyle(22)
        h_regMbb[i].SetXTitle("Mbb")
        h_regMbb[i].SetTitle("")
        h_regMbb[i].GetEntries()
	norm_h_regMbb =0.
        norm_h_regMbb = h_regMbb[i].Integral();
        h_regMbb[i].Scale(1/norm_h_regMbb)
        h_regMbb[i].Draw('P')
        h_regMbb[i].Fit("f2","","",100,150)
        f2.SetLineColor(3)
        f2.Draw('same')
        ...
        legend.AddEntry(h_Mbb[i],mbb_entry)
        legend.AddEntry(f1,f1_fitvalues)
        legend.AddEntry(h_regMbb[i],regmbb_entry)
 ...
 #   outfile.write(h_regMbb)
        histname="MBB_"+str(i)+"_"+str(int(_pt[i]))+"to"+str(int(_pt[i+1]))+".pdf"
        c1.SaveAs(histname)
        c1.Delete()
